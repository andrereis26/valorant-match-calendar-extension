...
(() => {
  ...
  // src/ui/matchListView.ts
  function requiredElement(selector) {
    const element = document.querySelector(selector);
    if (!element) throw new Error(`Missing UI element: ${selector}`);
    return element;
  }
  function mountMatchListView(platform) {
    const statusElement = requiredElement("#status");
    const matchesElement = requiredElement("#matches");
    const refreshButton = requiredElement("#refreshButton");
    const settingsButton = requiredElement("#settingsButton");
    const scheduleViewButton = requiredElement("#scheduleViewButton");
    const resultsViewButton = requiredElement("#resultsViewButton");
    const vctFilterButton = requiredElement("#vctFilterButton");
    const allFilterButton = requiredElement("#allFilterButton");
    ...
    settingsButton.addEventListener("click", () => platform.openSettings());
    ...
    vctFilterButton.addEventListener("click", () => {
      selectedFilter = "vct";
      void saveActiveMatchFilter(platform.storage, selectedFilter);
      void renderMatches();
    });
    ...
    function matchCard(match, config) {
      ...
      if (match.status === "upcoming" && match.hasStartTime) {
        ...
        addButton.addEventListener("click", (event2) => {
          ...
          platform.openLink(
            createCalendarUrl(match, config.durationMinutes || 120)
          );
        });
    ...
    async function renderMatches() {
      ...
      try {
        ...
        selectedFilter ??= normalizeMatchFilter(
          await loadActiveMatchFilter(platform.storage, config)
        );
        updateControls();
